...
RUN mkdir src && echo "fn main() {}" > src/main.rs && \
    cargo build --release && \
    rm -rf src target/release/kick-channel-role target/release/deps/kick_channel_role*
...
COPY --from=builder /app/target/release/kick-channel-role /usr/local/bin/
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=10s --retries=3 \
    CMD curl --fail --silent --max-time 2 \
        http://127.0.0.1:8094/kick-channel-role/health || exit 1

USER app:app

CMD ["kick-channel-role"]
