//! Job-polling worker. N worker tasks (`WORKER_CONCURRENCY`) run in
//! parallel; each claims a batch via `FOR UPDATE SKIP LOCKED` and dispatches
//! by job kind. Workers stop accepting new work on shutdown but finish
...
/// Reap in_progress jobs whose lock is older than this (worker crashed
/// mid-claim). Slightly above the longest legitimate sync.
