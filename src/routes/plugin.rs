...
//! role-config page (`/admin/{guild}/role/{role_id}`, Phase 6) and we own
//! the entire save UX. POST /config is a no-op kept around for contract
//! compliance: it is never called by iframe-mode plugins, but we still
//! verify the token so a leaked/stale call cannot ping us silently.
