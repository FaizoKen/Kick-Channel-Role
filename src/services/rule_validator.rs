...
//!
//! Mirrors [Form-Respondent-Role/src/schema.rs] `parse_role_config` in shape
//! and error-message style. Returns a clean `RuleTree` ready to persist as
//! `role_links.rule_tree` JSONB.
...
pub struct RuleTreeBody {
    /// Optional channel binding. `None` lets admins save a draft rule before
    /// they've connected a Kick channel — the role grants to nobody until
    /// a channel is bound.
    #[serde(default)]
    pub kick_channel_id: Option<i64>,
    #[serde(default)]
...
pub fn parse_rule_tree(body: RuleTreeBody) -> Result<ParsedRule, AppError> {
    ...
    Ok(ParsedRule {
        kick_channel_id: body.kick_channel_id,
        rule_tree: RuleTree {
...
mod tests {
    ...
    fn int_coerces_from_string() {
        let body = one_group(vec![input("sub_months_cumulative", "gte", json!("3"))]);
        let parsed = parse_rule_tree(body).unwrap();
        assert_eq!(parsed.rule_tree.groups[0].conditions[0].value, json!(3));
    }
        ...
        let body = RuleTreeBody {
            ...
            groups: vec![ConditionGroupInput {
                conditions: vec![ConditionInput {
                    target: "sub_months_cumulative".into(),
    ...
    #[test]
    fn string_list_normalizes_csv() {
        let body = one_group(vec![input("country_code", "in", json!("US,CA,GB"))]);
        let parsed = parse_rule_tree(body).unwrap();
        assert_eq!(
            parsed.rule_tree.groups[0].conditions[0].value,
            json!(["US", "CA", "GB"])
    ...
    fn regex_pattern_must_compile() {
        let body = one_group(vec![input("username", "regex", json!("(("))]);
        assert!(matches!(
            parse_rule_tree(body),
            Err(AppError::BadRequest(_))
        ));
