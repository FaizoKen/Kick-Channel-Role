...
//!
//! Events survive replica crashes / SIGTERM (durable, transactional); any
//! replica can pick up a job (`FOR UPDATE SKIP LOCKED`); transient upstream
//! failures retry with exponential backoff + jitter; permanently-failing
//! jobs land in a DLQ (`status = 'dead'`).
//!
//! Enqueue with `enqueue_*` (it INSERTs and conditionally `pg_notify`s in one
//! round-trip); the job worker (`tasks::job_worker`) claims and dispatches.
...
/// Postgres NOTIFY channel `enqueue` fires when an immediately-runnable job
/// is inserted. `tasks::job_listener` LISTENs and wakes the shared `Notify`.
pub const JOBS_CHANNEL: &str = "jobs_pending";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
// The `*Sync` suffix is the meaningful, intentional naming here (it mirrors
// the worker dispatch arms); the variant-names lint is a false positive.
...
pub async fn enqueue_channel_sync<'e, E>(executor: E, kick_channel_id: i64) -> Result<(), AppError>
where
    E: PgExecutor<'e>,
{
